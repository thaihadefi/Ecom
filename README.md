...
- **AI-Powered Support Assistant:** LLM-driven semantic analysis of support conversations (smart reply suggestions, draft response refinement, conversation summarization, and customer sentiment/emotion detection), powered by the Groq API (model set by `GROQ_MODEL`, default `openai/gpt-oss-20b`), separate from the real-time chat transport itself.
...
| `GROQ_API_KEY`, `GROQ_MODEL` | no | Admin AI assistant key and model (default `openai/gpt-oss-20b`) |
