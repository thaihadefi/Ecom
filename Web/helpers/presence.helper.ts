...
export const presenceCache = new NodeCache({ stdTTL: 3600, checkperiod: 600 });
const cache = presenceCache;
...
export const getLastSeen = async (role: PresenceRole, id: string): Promise<number | undefined> => {
  ...
  try {
    const doc = await modelFor(role).findById(id).select("lastSeenAt");
