...
import { invalidateProductCaches, invalidateArticleCaches, invalidateHomeBlockList } from "./metadata-cache.helper";
import { invalidateSettingCache } from "../configs/setting.config";
import { invalidateAdminAuthCache } from "../services/admin/auth.service";
...
export async function propagateMediaRename(
  ...
  updates.push(
    ...
    Product.updateMany({ "variants.image": oldPath }, { $set: { "variants.$[elem].image": newPath } }, { arrayFilters: [{ "elem.image": oldPath }] }),
    ...
    Product.updateMany({ "variants.image": rawOld }, { $set: { "variants.$[elem].image": rawNew } }, { arrayFilters: [{ "elem.image": rawOld }] }),
...
export async function propagateMediaDelete(filePath: string): Promise<void> {
  ...
  const rawTarget = filePath.replace(/^\/media\//, "");
  updates.push(
    Product.updateMany({ images: filePath }, { $pull: { images: filePath } }),
    Product.updateMany({ images: rawTarget }, { $pull: { images: rawTarget } }),
    Product.updateMany({ "variants.image": { $in: [filePath, rawTarget] } }, { $set: { "variants.$[elem].image": "" } }, { arrayFilters: [{ "elem.image": { $in: [filePath, rawTarget] } }] }),
    Review.updateMany({ images: filePath }, { $pull: { images: filePath } }),
    Review.updateMany({ images: rawTarget }, { $pull: { images: rawTarget } }),
    ChatMessage.updateMany({ files: filePath }, { $pull: { files: filePath } }),
    ChatMessage.updateMany({ files: rawTarget }, { $pull: { files: rawTarget } }),
    Blog.updateMany({ avatar: { $in: [filePath, rawTarget] } }, { $set: { avatar: null } }),
    CategoryProduct.updateMany({ avatar: { $in: [filePath, rawTarget] } }, { $set: { avatar: null } }),
    CategoryBlog.updateMany({ avatar: { $in: [filePath, rawTarget] } }, { $set: { avatar: null } }),
    AccountAdmin.updateMany({ avatar: { $in: [filePath, rawTarget] } }, { $set: { avatar: null } }),
    AccountUser.updateMany({ avatar: { $in: [filePath, rawTarget] } }, { $set: { avatar: null } })
  );
