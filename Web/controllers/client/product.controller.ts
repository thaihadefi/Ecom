  ...
  try {
    ...
    if (categoryId && '_id' in categoryDetail) {
      ...
      if (!req.cookies[viewed]) {
        void productService.incrementCategoryProductView(categoryId).catch((err) => console.error("incrementCategoryProductView error:", err));
...
export const detail = async (req: Request, res: Response) => {
  try {
    ...
    const viewed = `viewed_product_${productId}`;
    res.cookie(viewed, "true", {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production',
      sameSite: 'strict',
    ...
    if (!req.cookies[viewed]) {
      void productService.incrementProductView(productId).catch((err) => console.error("incrementProductView error:", err));
    ...
    const updatedHistory = [productId, ...productViewHistory.filter((id) => id !== productId)].slice(0, PRODUCT_DISPLAY_CONFIG.VIEWED_PRODUCTS_LIMIT);
