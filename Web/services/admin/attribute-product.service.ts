...
import { metadataCache, CACHE_KEYS, invalidateActiveAttributes } from "../../helpers/metadata-cache.helper";

export const getActiveAttributes = async (): Promise<IAttributeProduct[]> => {
  const cached = metadataCache.get<IAttributeProduct[]>(CACHE_KEYS.ACTIVE_ATTRIBUTES);
  if (cached) return cached;

  const list = await AttributeProduct.find({ deleted: false }).select("_id name type options").sort({ createdAt: "desc" });
  metadataCache.set(CACHE_KEYS.ACTIVE_ATTRIBUTES, list);
  return list;
};
