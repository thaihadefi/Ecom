...
export const deleteReviewById = async (id: string) => {
  ...
  if (review.userId) {
    invalidateUserDashboardCache(String(review.userId));
  }
...
export const deleteManyReviews = async (ids: string[]) => {
  const reviews = await Review.find({ _id: { $in: ids } }).select("productId userId");
  const affectedProductIds = [...new Set(reviews.map(r => String(r.productId)).filter(Boolean))];
  const affectedUserIds = [...new Set(reviews.map(r => String(r.userId)).filter(Boolean))];
  ...
  invalidateProductCaches();
  affectedUserIds.forEach(uId => invalidateUserDashboardCache(uId));
