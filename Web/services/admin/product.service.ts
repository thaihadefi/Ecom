...
export const updateProduct = async (id: string, data: IProductInput): Promise<{ success: boolean; status?: number; message: string; product?: IProduct | null }> => {
  ...
  invalidateProductCaches(productDetail.slug);
  if (data.slug && data.slug !== productDetail.slug) {
    invalidateProductCaches(String(data.slug));
  }
...
export const softDeleteManyProducts = (ids: string[]) => {
  invalidateProductCaches();
  return softDeleteMany(Product, ids, "product");
};
...
export const restoreManyProducts = (ids: string[]) => {
  invalidateProductCaches();
  return restoreMany(Product, ids, "product");
};
