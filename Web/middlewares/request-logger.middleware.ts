  ...
  res.on("finish", () => {
    ...
    if (IS_PRODUCTION) {
      console.log(JSON.stringify({
        level, type: "http_request", requestId,
        method: req.method, path: sanitizedPath,
        statusCode, durationMs, ip: req.ip,
        userAgent: req.headers["user-agent"] || "",
      }));
      return;
    }

    console.log(`${level.toUpperCase()} ${req.method} ${sanitizedPath} ${statusCode} ${durationMs}ms [${requestId}]`);
  });
